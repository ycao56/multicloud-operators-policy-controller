...
type SyncedPolicyMap struct {
	PolicyMap map[string]*policiesv1alpha1.TrustedContainerPolicy
	//Mx for making the map thread safe
	Mx sync.RWMutex
}

//GetObject used for fetching objects from the synced map
func (spm *SyncedPolicyMap) GetObject(key string) (value *policiesv1alpha1.TrustedContainerPolicy, found bool) {
	spm.Mx.Lock()
	defer spm.Mx.Unlock()
...
func (spm *SyncedPolicyMap) AddObject(key string, plc *policiesv1alpha1.TrustedContainerPolicy) {
	spm.Mx.Lock()
	defer spm.Mx.Unlock()
	//check if the map is initialized, if not initilize it
	if spm.PolicyMap == nil {
		spm.PolicyMap = make(map[string]*policiesv1alpha1.TrustedContainerPolicy)
	}
