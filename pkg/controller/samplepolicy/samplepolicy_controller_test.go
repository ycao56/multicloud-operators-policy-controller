...
func TestReconcile(t *testing.T) {
	...
	// Create a ReconcileTrustedContainerPolicy object with the scheme and fake client
	r := &ReconcileTrustedContainerPolicy{client: cl, scheme: s, recorder: nil}
...
func TestCheckUnNamespacedPolicies(t *testing.T) {
	var simpleClient kubernetes.Interface = testclient.NewSimpleClientset()
	common.Initialize(&simpleClient, nil)
	var samplePolicy = policiesv1alpha1.TrustedContainerPolicy{
		ObjectMeta: metav1.ObjectMeta{
			Name:      "foo",
			Namespace: "default",
		}}

	var policies = map[string]*policiesv1alpha1.TrustedContainerPolicy{}
...
func TestCheckViolationsPerNamespace(t *testing.T) {
	...
	var samplePolicySpec = policiesv1alpha1.TrustedContainerPolicySpec{
		MaxRoleBindingUsersPerNamespace:  1,
...
func TestConvertPolicyStatusToString(t *testing.T) {
	...
	samplePolicyStatus := policiesv1alpha1.TrustedContainerPolicyStatus{
...
func TestDeleteExternalDependency(t *testing.T) {
	...
	reconcileTrustedContainerPolicy := ReconcileTrustedContainerPolicy{client: mgr.GetClient(), scheme: mgr.GetScheme(), recorder: mgr.GetEventRecorderFor("trustedcontainerpolicy-controller")}
	reconcileTrustedContainerPolicy.deleteExternalDependency(&samplePolicy)
}
